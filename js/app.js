...
function renderDashboard(){
  ...
  if(donut.length){donutChart=new Chart($("donut"),{type:"doughnut",data:{labels:donut.map(d=>d.name),datasets:[{data:donut.map(d=>d.value),backgroundColor:donut.map(d=>d.color),borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,cutout:"68%",onClick:(e,els)=>{if(els&&els.length)drillCat(donut[els[0].index].id);},onHover:(e,els)=>{e.native.target.style.cursor=els&&els.length?"pointer":"default";},plugins:{legend:{display:false},tooltip:{callbacks:{label:(x)=>" "+fmt(x.raw)}}}}});}
